...
configs = []
for loss_scale_rn in [0.0, 1.0]:
    for loss_scale_sclip in [0.0, 1.0]:
        if loss_scale_rn != 0.0 or loss_scale_sclip != 0.0:
            configs.append(RunConfig(
                loss_calculate_classification_1k=True,
                loss_calculate_clip=True,
                loss_calculate_rn=True,
                loss_calculate_sclip = True,
                loss_calculate_tv=True,
                loss_scale_rn=loss_scale_rn,
                loss_scale_sclip=loss_scale_sclip,
                loss_scale_tv=0.2 * (loss_scale_rn + loss_scale_sclip),
                iterations=3000,
            ))
configs.append(RunConfig(
    loss_calculate_classification_1k=True,
...
for i, img_path in enumerate(img_paths):
    for ii, config in enumerate(configs):
        ...
        run_reconstructions(img_path=img_path, config=config,
                            text_logging_freq=50, img_logging_freq=100, reconstructions_per_img=4,
                            clip_suite=clip_suite, run_tags=["sclip", "gen2"])