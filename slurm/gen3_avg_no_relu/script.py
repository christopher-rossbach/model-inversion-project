...
for i, img_path in enumerate(img_paths):
    for model_name in ["resnet18", "resnet34", "resnet50", "resnet101", "resnet152", "vgg16", "vgg19", "clip"]:
        config = RunConfig(
                    loss_scale_rn=1.0,
                    loss_scale_tv=1.0,
                    loss_scale_avg_clip=1.0 if model_name == "clip" else 0.0,
                    loss_scale_avg_vgg16=1.0 if model_name == "vgg16" else 0.0,
                    loss_scale_avg_vgg19=1.0 if model_name == "vgg19" else 0.0,
                    loss_scale_avg_resnet18=1.0 if model_name == "resnet18" else 0.0,
                    loss_scale_avg_resnet34=1.0 if model_name == "resnet34" else 0.0,
                    loss_scale_avg_resnet50=1.0 if model_name == "resnet50" else 0.0,
                    loss_scale_avg_resnet101=1.0 if model_name == "resnet101" else 0.0,
        ...
        run_reconstructions(img_path=img_path, config=config,
                            ...
                            clip_suite=clip_suite, run_tags=["ffhq", "only_16", "avg_nrl", "avg_nrl_" + model_name])